      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 md:gap-8">
        {items.map((item, idx) => (
          <div key={idx} className="flex flex-col group bg-white dark:bg-[#1a1a1a] shadow-[0_2px_4px_rgba(0,0,0,0.08)] hover:shadow-[0_8px_20px_rgba(0,0,0,0.1)] dark:shadow-none dark:border dark:border-[#333] h-full transition-all">
            <div className="w-full aspect-[4/3] sm:aspect-video md:h-48 overflow-hidden relative">
              <Image 
                src={item.image} 
                alt={item.title} 
                fill
            ...
            <div className="p-3 sm:p-5 md:p-6 flex flex-col flex-grow">
              <h3 className="text-sm sm:text-lg md:text-xl font-semibold mb-2 sm:mb-3 text-[#242424] dark:text-white leading-snug line-clamp-2">{item.title}</h3>
              <p className="text-[10px] sm:text-xs md:text-sm text-[#505050] dark:text-gray-300 mb-4 sm:mb-6 flex-grow leading-relaxed line-clamp-3">{item.description}</p>
              <Link 
                href={item.link} 
                className="inline-flex items-center text-[#00d4ff] dark:text-[#4da3ff] font-semibold text-[10px] sm:text-sm hover:underline w-fit mt-auto group/btn"
